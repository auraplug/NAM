...
def normalize_shifted_accumulator(
    acc_scaled: int,
    shift: int,
) -> int:

    # The right shift that produces acc_scaled is part of the
    # Q-format: it removes 'shift' bits from the accumulator
    # fractional part as well.
    #
    # Q4.28 -> Q2.14 normalization must therefore divide by
    #
    #     Q_SCALE >> shift
    #
    # instead of Q_SCALE.
    ...
    if shift > Q_FRAC:
        raise ValueError(
            f"accumulator shift {shift} exceeds Q fractional bits {Q_FRAC}"
        )

    denominator = Q_SCALE >> shift
...
def write_binary(
    ...
    if len(q_values) != EXPECTED_PARAMETER_COUNT:
        raise ValueError(
            f"Invalid parameter count: "
            f"{len(q_values)} != {EXPECTED_PARAMETER_COUNT}"
        )
...
class Int32BF592Reference:
    ...
    def process_layer(
        ...
        for out_ch in range(CHANNELS):
            ...
            z = normalize_shifted_accumulator(
                acc_scaled,
                shift,
            )
        ...
        for out_ch in range(CHANNELS):
            ...
            residual = normalize_shifted_accumulator(
                acc_scaled,
                shift,
            )
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            print(
                f"Byte-exact payload  : "
                f"{'PASS' if binary_pass else 'FAIL'}"
            )

        except ValidationError as exc:
            print(
                "Byte-exact payload  : FAIL"
            )
            print(
